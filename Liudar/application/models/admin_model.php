...
class Admin_model extends CI_Model {
    ...
    public function hotelOrder($id){
        $this -> db -> select('*');
        $this -> db -> from('t_admin');
        $this -> db -> join('t_order','t_order.hotel_id = t_admin.hotel_id');
        $this -> db -> where('t_admin.admin_id',$id);
        return $this -> db -> get() -> result();
    }
    public function localOrder($id){
        $this -> db -> select('*');
        $this -> db -> from('t_admin');
        $this -> db -> join('t_order','t_order.local_id = t_admin.local_id');
        $this -> db -> where('t_admin.admin_id',$id);
        return $this -> db -> get() -> result();
    }
